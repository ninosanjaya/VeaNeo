...
func state_input(event : InputEvent):
	if(event.is_action_pressed("up")) && Global.talking== false && Global.dog == false && Global.control_metal == false:
		jump()
		
		
	if(event.is_action_pressed("dash") && can_dash == true && can_dash2 == true) && Global.talking == false && Global.dog == false && Global.control_metal == false:
		dash()
