...
const SINGLE_OBJECT_PARTICLE = preload("res://Objects/single_object_particle.tscn")
#var air_jump = false
...
@onready var single_object_particle = $Single_object_particle
var fire = null

func handle_attack():
	
	#flamethrower
	if (Input.is_action_pressed("attack"))  and (Global.talking == false):
		#state_machine.current_state = attack_state
		playback.travel(attack_animation)
		attack_timer_pp.start()
		can_attack = false
		#single_object_particle.visible = true
		if right == true:
			#position.x += 5 
			#fire = SINGLE_OBJECT_PARTICLE.instantiate()
			#3held_item_position.add_child(fire)
			#add_child(fire)
			Global.right_global = true
			single_object_particle.position = Vector2(28,-22)
			#single_object_particle.scale = Vector2(1,1)
			single_object_particle.visible = true
			#fire = null
			#held_item_position.remove_child(fire)
		elif right == false:
			#position.x -= 5
			#fire = SINGLE_OBJECT_PARTICLE.instantiate()
			#held_item_position_2.add_child(fire)
			#add_child(fire)
			Global.right_global = false
			single_object_particle.position = Vector2(-28,-22)
			#single_object_particle.scale = Vector2(-1,1)
			single_object_particle.visible = true
			#fire = null
			#single_object_particle.position = Vector2(-28,-22)
			#single_object_particle.scale = Vector2(-1,1)
			#held_item_position_2.remove_child(fire)
	elif !(Input.is_action_pressed("attack")):
	

			#held_item_position.remove_child(fire)
			#held_item_position_2.remove_child(fire)
			single_object_particle.visible = false
			#animation_tree.set("parameters/move/blend_position", direction.x)
		
	#syringe, absorb liquid & spray liquid for puzzle, but hurt/attack enemy too
	if (Input.is_action_pressed("skill"))  and (Global.talking == false):
		playback.travel(attack_animation)
		attack_timer_pp.start()
		can_attack = false
		#single_object_particle.visible = true
